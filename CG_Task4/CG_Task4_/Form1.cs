...
namespace CG_Task4_
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void DrawLineButton_Click(object sender, EventArgs e)
        {
            ...
            {
                firstArrayOfPoints = new Point[FirstLineTable.RowCount - 1];
                secondArrayOfPoints = new Point[SecondLineTable.RowCount - 1];


                int iter = 0;
                foreach (DataGridViewRow row in FirstLineTable.Rows)
                {
                ...
                iter = 0;
                foreach (DataGridViewRow row in SecondLineTable.Rows)
                {
                    if (row.Cells[0].Value != null)
                    {
                        secondArrayOfPoints[iter] = new Point(int.Parse(row.Cells[0].Value.ToString()), int.Parse(row.Cells[1].Value.ToString()));
                        iter++;
                    }
                }
